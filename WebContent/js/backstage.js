...
$(function () {
    ...
    oFileInput.Init("myFile", "/hotelSystem/room/addRoom");
    checkid();
		...
		$.ajax({ 
			...
			success: function (XMLHttpRequest) { 
				...
				if(XMLHttpRequest=="ok"){
					 window.location.href="http://localhost:8080/hotelSystem/room/getAll";
...
function checkRoom(room) {
	...
	if(roomId.length==0){
		...
		//验证数据库是否有房间号
		$.ajax({ 
		     url: '/hotelSystem/room/checkRoomId?roomid='+roomId, 
		     type: 'POST', 
		     async: true, //异步
		     data:"",
		     cache: false, //禁止缓存
...
function checkImage(finds) {
	if(finds.find("img").length<1){
		...
		$("#upload").next().siblings("span").remove();
		$("#upload").next().after('<span class="font">*图片至少一张</span>');
		return false;
	}else{
		$("#upload").next().siblings("span").remove();
		return true;
